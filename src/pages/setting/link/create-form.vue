<template>
    <div>
        <Modal width="600" :mask-closable="false" v-model="showCreate" title="新建标签" :loading="creating" @on-ok="handleCreate" @on-cancel="handleCancel">
            <Form ref="create" :model="createData" :rules="createRules" label-position="right" :label-width="100">
                <FormItem label="名称：" prop="user_name">
                    <Input v-model="createData.user_name" placeholder="请输入用户名" />
                </FormItem>
                <FormItem label="链接地址：" prop="real_name">
                    <Input v-model="createData.real_name" placeholder="请输入真实姓名" />
                </FormItem>
                <FormItem label="打开方式：" prop="phone">
                    <Select v-model="createData.open_type" placeholder="请选择" clearable>
                        <Option value="1">本窗口打开</Option>
                        <Option value="2">新标签打开</Option>
                    </Select>
                </FormItem>
                <FormItem label="描述：" prop="phone">
                    <Input v-model="createData.phone" pl    aceholder="请输入手机号" />
                </FormItem>
