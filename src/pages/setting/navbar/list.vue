<template>
    <div class="i-table-no-border">
        ...
        >
            <template slot-scope="{ row }" slot="is_main">
                <Tag v-if="row.is_main === 0" color="default">否</Tag>
                <Tag v-if="row.is_main === 1" color="success">是</Tag>
            </template>
            ...
            <template slot-scope="{ row, index }" slot="action">
                <a @click="handleItemCreate(true)">菜单管理</a>
                <Divider type="vertical" />
                <a @click="handleOpenUpdateCreate(true, row.id)">编辑</a>
                <Divider type="vertical" />
                <a @click="handleDelete(true, row.id)">删除</a>
            </template>
        </Table>
        ...
        <!-- 添加菜单 -->
        <create-item-form ref="createItemForm" @on-create-form="handleItemCreate" />
    </div>
</template>
<script>
    import searchForm from '@/components/searchform';
    import createForm from './create-form';
    import {NavigationIndex, NavigationSearch, NavigationUpdate} from "@api/navbar";
    import CreateItemForm from "./item/item-form";
    export default {
        components: {
            CreateItemForm,
            searchForm,
        ...
        methods: {
            ...
            handleItemCreate (status) {
                this.$refs.createItemForm.handleItemCreate(status);
            },
